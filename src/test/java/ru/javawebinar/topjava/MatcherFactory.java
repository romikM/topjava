...
public class MatcherFactory {
    ...
    public static <T> Matcher<T> usingEqualsComparator(Class<T> clazz) {
        return usingAssertions(clazz,
                (act, exp) -> assertThat(act).isEqualTo(exp),
                (act, exp) -> assertThat(act).isEqualTo(exp));
    }

    public static <T> Matcher<T> usingIgnoringFieldsComparator(Class<T> clazz, String... fieldsToIgnore) {
        return usingAssertions(clazz,
                (act, exp) -> assertThat(act).usingRecursiveComparison().ignoringFields(fieldsToIgnore).isEqualTo(exp),
                (act, exp) -> assertThat(act).usingRecursiveFieldByFieldElementComparatorIgnoringFields(fieldsToIgnore).isEqualTo(exp)
        );
    }
